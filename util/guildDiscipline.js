...
module.exports = class GuildDiscipline {
    ...
    async finalize () {
        ...
        switch (this.type) {
            case 'warn':
                msg += ":warning: **__FORMAL WARNING ISSUED__** :warning: \n\n"
                ...
                break;
            case 'discipline':
                msg += ":octagonal_sign: **__DISCIPLINE ISSUED__** :octagonal_sign: \n\n"
                msg += "Your recent conduct has necessitated disciplinary action. You are being issued discipline for the following: \n"
                msg += `**Rule number(s) violated:** ${this.rules.join(", ")}` + "\n"
                msg += `**Further Information:** ${this.reason}` + "\n\n"
                ...
                break;
            case 'mute':
                msg += ":mute: **__MUTE ISSUED__** :mute: \n\n"
                msg += "Your recent conduct is causing problems for other members in the community. Therefore, for the safety of others, you are being issued a mute for the following: \n"
                msg += `**Rule number(s) violated:** ${this.rules.join(", ")}` + "\n"
                msg += `**Further Information:** ${this.reason}` + "\n\n"
                msg += "Staff would like to see you work on this concern and learn from this encounter. Staff may need to issue more severe discipline if this happens again."

                msg2 += "**Mute** \n"
                msg2 += `You have been muted from the guild ${this.duration === 0 ? 'until staff manually remove the muted role from you. You will not have access to the rest of the guild until the mute is removed.' + "\n\n" : `until ${moment().add(this.duration, 'minutes').format("LLLL Z")}`} (${this.duration} minutes from now). You will not have access to the rest of the guild until the mute expires.` + "\n\n"
