...
class RichMenu extends RichDisplay {
	...
	_paginate() {
		...
		super.addPage(embed => {
                        var emojis = [this.emojis.zero, this.emojis.one, this.emojis.two, this.emojis.three, this.emojis.four, this.emojis.five, this.emojis.six, this.emojis.seven, this.emojis.eight, this.emojis.nine];
			for (let i = 0, option = this.options[i + (page * 10)]; i + (page * 10) < this.options.length && i < 10; i++, option = this.options[i + (page * 10)]) {
				embed.addField(`${emojis[i]} : ${option.name}`, option.body, option.inline);
			}
			return embed;
		});
