...
module.exports = class extends Command {
    ...
    constructor(...args) {
        super(...args, {
            ...
            description: 'Starts the discipline wizard to issue either a warning, discipline, mute, suspension, or ban to a user.',
    ...
    async run(message, [user]) {
        ...
        var type = await message.awaitReply(`:question: What kind of discipline are you issuing this member? Please reply with one of the following: warn, discipline, mute, tempban, ban. You have 5 minutes to respond.`, 300000);
        if (!type)
            return message.send(`:x: The wizard timed out and was canceled.`);
        if (["warn", "discipline", "mute", "tempban", "ban"].indexOf(type) === -1)
            return message.send(`:x: You provided an invalid disciplinary action. Please try the wizard again.`);
