...
module.exports = class extends Command {
	...
	async run(msg, [when, member, ...reason]) {
		if (member.id === msg.author.id) throw 'It is not necessary to mute yourself';
		if (member.id === this.client.user.id) throw 'Am I talking too much?';
		...
		if (when) {
			...
			return msg.sendMessage(`${member.user.tag} was muted for ${Duration.toNow(when)}.${reason ? ` because of reason: ${reason}` : ''}`);
		}

		return msg.sendMessage(`${member.user.tag} was muted until the unmute command is used.${reason ? ` because of reason: ${reason}` : ''}`);
	}
